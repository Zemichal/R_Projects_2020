...
ui <- fluidPage(
  ...
  mainPanel(tabsetPanel(
    ...
    tabPanel("Nine Box Chart", plotOutput("nineB", width = "100%", height = "700px")))),
  fluidRow(column(2,wellPanel(
    h5("Set these Parameters First then Update Chart"),
    ...
    selectInput("fv", "Please Select Vertical Facet", choices =c("Level", "Performance","Benchmark Potential","NULL"), selected = "NULL"),
    selectInput("fh", "Please Select Horizontal Facet", choices =c("Level", "Performance","Benchmark Potential","NULL"), selected = "NULL")),
  ...
  column(3,wellPanel(
    ...
    h5("When setting the Midpoint for the H.Chart use the inverse of the number"),
    numericInput("mp", "Set the Midpoint Value", 15))),
  
  fluidRow(column(4,wellPanel(
    h5("Use this to set the Range for the H. Chart"),
    numericRangeInput("scalexr", "Please Select Range For X Axis",
                      value = c(-115,115)),
    h5("Use this to remove ticks on X Axis"),
    ...
    h5("Use this to set the Range for the V. Chart"),
    numericRangeInput("scaleyr", "Please Select Range For Y Axis",
                      value = c(-115,115)),
    h5("Use this to remove ticks on Y Axis"),
    ...
    downloadButton('downloadPlot3', 'Download Nine Box')
  )))
...
server <- function(input, output, session) {
  ...
  facet_v_input <- eventReactive(input$button,{
    switch(input$fv,
           "Performance" = as.character(eb4[,"perf"]),"Level" = as.character(eb4[,"lvls"]),"Benchmark Potential" = as.character(eb4[,"...46"]),"NULL" = " ")
  })
  ...
  plotOutput1 <- function(){
    ggplot(data = eb4) +
      ...
      facet_grid(facet_h_input()~facet_v_input()) +
      labs(
        x= input$xl,                       
        y= input$yl,
        color = " ") + 
  ...
  plotOutput3 <- function(){
    ggplot(data = eb4) +
      ...
      labs(
        ...
        y= input$yl,
        color = " ") + 
      geom_text_repel(mapping = aes(x=axis_x_input(), y=axis_y_input(),label=as.character(...3)))+
      guides(size = FALSE)+
      scale_x_continuous(limits = input$scalexr,breaks = seq(from = input$scalex1, to = input$scalex2, by = input$byx)) +
      scale_y_continuous(limits = input$scaleyr,breaks = seq(from = input$scaley1, to = input$scaley2, by = input$by)) +
      theme( panel.background = element_rect(fill = "transparent"), # bg of the panel
